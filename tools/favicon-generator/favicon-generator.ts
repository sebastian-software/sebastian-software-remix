...
const pngSettings: sharp.PngOptions = { compressionLevel: 9, progressive: true, quality: 100 }
...
async function faviconPngGenerator(svgImage: sharp.Sharp, filename: string, size: number) {
  const pngBuffer = await svgImage.
...
async function run() {
  ...
  await faviconPngGenerator(iconSvg.clone(), WEB_MANIFEST_192, WEB_MANIFEST_192_SIZE);
